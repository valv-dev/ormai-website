...
const TABS = {
  mcp: {
    label: "MCP",
    command: "npx riptide-mcp@latest",
    placeholder: "MCP config example",
    Icon: McpIcon,
  },
  ts: {
    label: "Typescript",
    command: "npm i riptide",
  ...
  return (
    <div className="w-full flex flex-col items-center gap-5">
      ...
      <div className="w-full flex flex-col">
        <CopyCommand key={active} command={tab.command} />
        <div className="w-full h-[400px] rounded-b-sm ring-[1px] ring-border bg-bg flex items-center justify-center">
          <span className="text-xs font-medium opacity-30">{tab.placeholder}</span>
        </div>
      </div>
    </div>
  )
